...
## 五、古籍引文能力（v0.3.0 新增 · 面向体验而非准确率）

目标：不追 benchmark 分数，转向**情绪价值与差异化体验**——产出偶尔引一句原文，给用户"有传承、有依据、被郑重对待"的质感。

- `rag/build-index.mjs`：从青囊 `outputs/citations`（17 本、823 段）抽取**可引用句段 5070 条**（24–72 字、去重、按书上限 500），产出仓库自带 `rag/citations.json`（1.38MB，不依赖青囊路径）
- `rag/retrieve.mjs`：门派路由（八字→滴天髓/子平真诠/穷通宝鉴/渊海子平；六爻→卜筮正宗/增删卜易；奇门→奇门遁甲统宗；紫微→紫微斗数全书/果老星宗）+ 可读性加权（句末完整 +2、半角逗号 −3、数字堆砌 −2、短句略优先）
- MCP 工具 `cite_lookup`（工具数 8 → 9）
- `prompts/system-prompt.md` 重写为「体验优先版」：先接住情绪再论断；引文纪律（频次 1–2 处、位置在核心论断后、必须白话转译+标出处、禁止编造）
